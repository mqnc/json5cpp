...
Token consumeCharacter (State s) {
  switch (s) {
    ...
    case State::decimalInteger: {
        ...
        return newToken(TokenType::numeric, sign * std::stod(buffer));
    } break;
    ...
    case State::string: {
        ...
        case '"':
            if (doubleQuote) {
                read();
                return newToken(TokenType::string, buffer);
            };
...
void consumeToken (State s) {
  switch (s) {
    ...
    case State::beforePropertyName: {
        switch (token.type) { default: break;
